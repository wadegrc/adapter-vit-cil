...
class VisionTransformer(nn.Module):
    ...
    def __init__(
            ...
            class_token=True, no_embed_class=True, fc_norm=None, drop_rate=0., attn_drop_rate=0., drop_path_rate=0.,
       ...
       # ]))
        ...
        self.pos_embed = nn.Parameter(torch.randn(1, 197, embed_dim) * .02)
        self.pos_drop = nn.Dropout(p=drop_rate)

        self.prompt_pool = prompt_pool
        self.head_type = head_type
        self.use_prompt_mask = use_prompt_mask
        self.adapter = Adapter(embed_dim, embedding_key=embedding_key) 
        if prompt_length is not None and pool_size is not None and prompt_pool: 
        ...
        #self.head = []
        #for i in range(10):
        #    self.fc_norm.append(norm_layer(embed_dim) if use_fc_norm else nn.Identity())
        #    self.head.append(nn.Linear(self.embed_dim, num_classes) if num_classes > 0 else nn.Identity())
        if weight_init != 'skip':
    ...
    def forward_features(self, x, task_id=-1, cls_features=None, train=False):
        ...
        if hasattr(self, 'promp'):
            #bias = self.meta_net(cls_features)
            if self.use_prompt_mask and train:
                start = task_id * self.prompt.top_k
                end = (task_id + 1) * self.prompt.top_k
                single_prompt_mask = torch.arange(start, end).to(x.device)
                prompt_mask = single_prompt_mask.unsqueeze(0).expand(x.shape[0], -1)
                meta_idx = torch.arange(0,1).to(x.device)
                meta_idx = meta_idx.unsqueeze(0).expand(x.shape[0], -1)
                if end > self.prompt.pool_size:
                    prompt_mask = None
            else:
                prompt_mask = None
                meta_idx = None
            res = self.prompt(x, prompt_mask=prompt_mask, cls_features=cls_features, meta_mask=meta_idx)
    ...
    def forward_head(self, res, pre_logits: bool = False,train=False, task_id=None, cls_features=None):
        x = res['x']
        if train:
            adapter_mask = torch.arange(task_id,task_id+1).to(x.device)
            adapter_mask = adapter_mask.unsqueeze(0).expand(x.shape[0], -1)
        else:
            adapter_mask = None
        res = self.adapter(x,adapter_mask)
        ratio = 0.6
        x = ratio*res['bias']+(1-ratio)*x
        x = self.norm(x)
        ...
        #if train is not True and task_id == 1:
            #print(res['adapter_idx'])
        res['pre_logits'] = x
        #self.fc_norm[task_id].to(x.device)
        x = self.fc_norm(x)
        #self.head[task_id].to(x.device)
    ...
    def forward(self, x, task_id=-1, cls_features=None, train=False):
        ...
        res = self.forward_head(res, train=train, task_id = task_id, cls_features=cls_features)
        #if train is not True and task_id == 6:
        #    print(res['adapter_idx'])
        return res
