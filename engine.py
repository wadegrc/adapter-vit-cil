...
def train_one_epoch(model: torch.nn.Module, original_model: torch.nn.Module, 
    ...
    for input, target in metric_logger.log_every(data_loader, args.print_freq, header):
        ...
        if args.pull_constraint and 'reduce_sim' in output:
            #loss = loss - args.pull_constraint_coeff * output['reduce_sim']+0.4*output['unsim']
            loss = loss -  output['reduce_sim']+ 0.1*output['unsim']
        ...
        #print(model.module.adapter.adapter_key.grad.data)
        if task_id >= 0:
            #for i, d in enumerate(model.module.adapter.adapter_key.parameters()):
            #    if i != task_id:
            #        d.grad.data.zero_()
            for i in range(10):
                if i != task_id:
                    model.module.adapter.adapter_key.grad.data[i].zero_()
        #if task_id >= 0:
        #    for i,d in enumerate(model.module.adapter.adapters[0].parameters()):
        #        print(d.data)
            #print(model.module.adapter.adapter_key.grad)
        optimizer.step()
...
def train_and_evaluate(model: torch.nn.Module, model_without_ddp: torch.nn.Module, original_model: torch.nn.Module, 
    ...
    for task_id in range(args.num_tasks):
       # Transfer previous learned prompt params to the new prompt
        if args.distributed:
            #model.module.prompt.update_meta_net(task_id,device)
            model.module.prompt.to(device)
        else:
            model.prompt.update_meta_net(task_id,device)
